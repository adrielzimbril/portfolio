...
export async function getAllChangelog(options: Partial<GetAllChangelogOptions> = {}): Promise<Changelog[]> {
  ...
  return Promise.resolve(
    allChangelogs
      .filter(
        (entry) =>
          entry.published === published && (!locale || entry.locale === locale) && (!type || entry.type === type),
      )
      .sort((a, b) => (sort === SortOrder.ASC ? a.date.localeCompare(b.date) : b.date.localeCompare(a.date)))
      .slice(0, limit),
  )
}
...
export async function getChangelogByVersion(
  ...
  return Promise.resolve(
    allChangelogs.find(
      (entry: Changelog) =>
        entry.version === version && (!locale || entry.locale === locale) && entry.published === true,
    ) ?? null,
  )
}
...
export async function getChangelogTypeCounts(
  ...
  return Promise.resolve({
    all: entries.length,
    milestone: entries.filter((e) => e.type === ChangelogItemType.MILESTONE).length,
    feature: entries.filter((e) => e.type === ChangelogItemType.FEATURE).length,
    fix: entries.filter((e) => e.type === ChangelogItemType.FIX).length,
    improvement: entries.filter((e) => e.type === ChangelogItemType.IMPROVEMENT).length,
  })
}
