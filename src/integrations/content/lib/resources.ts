...
export async function getAllResources(options: Partial<GetAllResourcesOptions> = {}): Promise<Resource[]> {
  ...
  return Promise.resolve(
    allResources
      .filter((resource) => resource.published === published && resource.locale === locale)
...
export async function getBestResourcesLink(options: Partial<GetAllResourcesOptions> = {}): Promise<ResourcePreview[]> {
  const { published, locale, pageSlug, sort, limit } = {
    published: true,
    locale: undefined,
    pageSlug: undefined,
...
export async function getResourceBySlug(
  ...
  return Promise.resolve(
    allResources.find((resource: Resource) => resource.slug === slug && (!locale || resource.locale === locale)) ??
      null,
  )
}

export async function getResourceById(id: number, options?: { locale?: string }): Promise<Resource | null> {
  const { locale } = options ?? {}
  return Promise.resolve(
    allResources.find(
      (resource: Resource) =>
        resource.id === id && (!locale || resource.locale === locale) && resource.published === true,
    ) ?? null,
  )
}
...
export async function getResourceWithAdjacent(
  ...
  const filteredResources = allResources.filter(
    (resource: Resource) => resource.published === true && (!locale || resource.locale === locale),
  )
  ...
  const currentIndex = sortedResources.findIndex((resource: Resource) => resource.slug === slug)
  ...
  const beforeResources = beforeCount > 0 ? sortedResources.slice(currentIndex - beforeCount, currentIndex) : []

  const afterResources = afterCount > 0 ? sortedResources.slice(currentIndex + 1, currentIndex + 1 + afterCount) : []
...
export async function getAllResourceSlugs(options: Partial<GetAllResourceSlugsOptions> = {}): Promise<string[]> {
  ...
  return Promise.resolve(
    allResources
      .filter((resource) => resource.published === published && (!locale || resource.locale === locale))
      .sort((a, b) => (sort === "asc" ? a.id - b.id : b.id - a.id))
...
export async function getFilteredResources(
  ...
  return Promise.resolve(
    resources.filter((resource) => {
      // Filter by tag
      if (options.tag && !resource.tags.find((tag) => tag.slug === options.tag)) {
        return false
      }
      ...
      if (options.search) {
        ...
        const contentMatch = resource.excerpt?.toLowerCase().includes(searchLower) || false
...
export async function getAllResourceTags(): Promise<string[]> {
  const allTags = allResources.flatMap((resource) => resource.tags.map((tag) => tag.slug))
