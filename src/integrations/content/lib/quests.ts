...
export async function getAllQuests(options: Partial<Options> = {}): Promise<Quest[]> {
...
export async function getQuestBySlug(slug: string, options?: { locale?: string }): Promise<Quest | null> {
  const { locale } = options ?? {}
  return Promise.resolve(allQuests.find((quest) => quest.slug === slug && quest.published === true) ?? null)
}
...
export async function getQuestWithAdjacent(
  ...
  const currentIndex = sortedQuests.findIndex((quest: Quest) => quest.slug === slug)
  ...
  // Retrieve the quests
  const beforeQuests = beforeCount > 0 ? sortedQuests.slice(currentIndex - beforeCount, currentIndex) : []

  const afterQuests = afterCount > 0 ? sortedQuests.slice(currentIndex + 1, currentIndex + 1 + afterCount) : []
...
export function isSubmissionClosed(submissionDeadline: string, questEnd: string): boolean {
  const now = Date.now()
  return now > new Date(submissionDeadline).getTime() || now > new Date(questEnd).getTime()
}

export function isResultsPublished(questEnd: string, resultsPublished: boolean): boolean {
