...
export async function getAllProjects(options: Partial<GetAllProjectsOptions> = {}): Promise<Project[]> {
  ...
  return Promise.resolve(
    allProjects
      .filter((project) => project.published === published && (!locale || project.locale === locale))
      .sort((a, b) =>
...
export async function getProjectBySlug(
  ...
  return Promise.resolve(
    allProjects.find(
      (project: Project) =>
        project.slug === slug && (!locale || project.locale === locale) && project.published === true,
    ) ?? null,
  )
}
...
export async function getProjectWithAdjacent(
  ...
  const filteredProjects = allProjects.filter(
    (project: Project) => project.published === true && (!locale || project.locale === locale),
  )
  ...
  const currentIndex = sortedProjects.findIndex((project: Project) => project.slug === slug)
  ...
  const beforeProjects = beforeCount > 0 ? sortedProjects.slice(currentIndex - beforeCount, currentIndex) : []

  const afterProjects = afterCount > 0 ? sortedProjects.slice(currentIndex + 1, currentIndex + 1 + afterCount) : []
...
export async function getAllProjectSlugs(options: Partial<GetAllProjectSlugsOptions> = {}): Promise<string[]> {
...
export async function getFilteredProjects(
  ...
  return Promise.resolve(
    projects.filter((project) => {
      // Filter by category
      if (options.category && project.categories.find((cat) => cat.slug === options.category)) {
        return false
      }

      // Filter by tag
      if (options.tag && !project.tags.find((tag) => tag.slug === options.tag)) {
        return false
      }
      ...
      if (options.search) {
        ...
        const contentMatch = project.excerpt?.toLowerCase().includes(searchLower) || false
...
export async function getAllProjectCategories(): Promise<string[]> {
  const categories = allProjects.map((project) => project.categories.map((cat) => cat.slug))
...
export async function getAllProjectTags(): Promise<string[]> {
  const allTags = allProjects.flatMap((project) => project.tags.map((tag) => tag.slug))
