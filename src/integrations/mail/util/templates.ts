...
export async function getTemplate<T extends TemplateId>({
  ...
  const templateKey = templateId as Exclude<keyof typeof mailTranslations, "common" | "basic">
  const templateTranslations = mailTranslations[templateKey]

  const subject = templateTranslations && "subject" in templateTranslations ? templateTranslations.subject : ""
