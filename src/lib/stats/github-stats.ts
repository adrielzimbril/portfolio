...
import type { GitHubStats, ContributionData, ContributionDay, ContributionWeek } from "@/lib/stats/types"
import { getGitHubConfig } from "@/config"

// GitHub configuration
const { token: GITHUB_TOKEN, username: GITHUB_USERNAME, repo: GITHUB_REPO, branch: GITHUB_BRANCH } = getGitHubConfig()
...
export async function getGitHubStats(): Promise<GitHubStats> {
  return unstable_cache(
    async () => {
      if (!GITHUB_TOKEN) {
        logger.warn("[GitHub Stats] GITHUB_TOKEN not set, returning empty stats")
        return getEmptyGitHubStats()
      }

      try {
        const [repoStats, contributions] = await Promise.all([fetchRepoStats(GITHUB_BRANCH), fetchContributions()])
...
async function fetchRepoStats(branch?: string) {
  ...
  if (!response.ok) {
    logger.error("[GitHub Stats] Failed to fetch GitHub repo stats:", response.status)
  ...
  if (!data.data) {
    ...
    logger.error("[GitHub Stats] Full response:", JSON.stringify(data, null, 2))
    return null
  }
...
async function fetchContributions(): Promise<ContributionData> {
  ...
  try {
    ...
    if (!response.ok) {
      logger.error("[GitHub Stats] Failed to fetch contributions:", response.status)
...
export async function getGitHubReleases() {
  if (!GITHUB_TOKEN) {
    logger.warn("[GitHub Releases] GITHUB_TOKEN not set, returning empty releases")
    return []
  }
  ...
  try {
    ...
    if (!response.ok) {
      logger.error("[GitHub Releases] Failed to fetch releases:", response.status)
      return []
    }
