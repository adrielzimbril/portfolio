...
// Configuration
const SITE_URL = ConfigValue.NEXT_PUBLIC_SITE_URL || "https://www.adrielzimbril.com"
const PAGESPEED_API_KEY = ConfigValue.PAGESPEED_API_KEY

// Cache duration in seconds (default 10 days = 864000 seconds)
const LIGHTHOUSE_CACHE_DAYS = parseInt(ConfigValue.LIGHTHOUSE_CACHE_DAYS || "10", 10)
...
export async function getLighthouseStats(): Promise<LighthouseStats> {
  return unstable_cache(
    async () => {
      if (!SITE_URL) {
        logger.warn("[Lighthouse Stats] SITE_URL not set, returning empty stats")
        return getEmptyLighthouseStats()
      }

      try {
        const [mobile, desktop] = await Promise.all([fetchLighthouseScores("mobile"), fetchLighthouseScores("desktop")])
        ...
        logger.error("[Lighthouse Stats] Error fetching Lighthouse stats:", error)
        return getEmptyLighthouseStats()
      }
    },
    ["lighthouse-stats"],
...
async function fetchLighthouseScores(strategy: "mobile" | "desktop"): Promise<LighthouseScores> {
