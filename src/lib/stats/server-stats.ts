...
export async function getServerStats(locale?: string): Promise<ServerStats> {
  ...
  const postTitleMap = new Map(allPosts.map((post) => [post.slug, post.title]))
  const postCoverMap = new Map(allPosts.map((post) => [post.slug, post.cover || ""]))

  // Fetch total views from page_counters
  const { data: pageCounters } = await supabase.from("page_counters").select("total_views")

  const totalViews = pageCounters?.reduce((sum, pc) => sum + (pc.total_views || 0), 0) || 0
  ...
  const { data: allReactions } = await supabase.from("reactions").select("reaction_type")
  ...
  const topViewedThoughts: ThoughtMetric[] =
    topViewedData?.map((item) => ({
      ...
      title: postTitleMap.get(item.slug || "") || item.slug.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
  ...
  const topReactedThoughts: ThoughtMetric[] = Object.entries(reactionCounts)
    ...
    .map(([slug, count]) => ({
      slug,
      title: postTitleMap.get(slug) || slug.replace(/-/g, " ").replace(/\b\w/g, (l) => l.toUpperCase()),
      count,
    }))
