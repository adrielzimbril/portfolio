export interface Product {
  id: string;
  title: string;
  description: string;
  image: string;
  price: number; // Price in USD
  officialPrice?: number; // Official price in USD
  currency: string;
  primaryTag: string;
  tags: string[];
  duration: string;
  isShared: boolean;
  featured?: boolean;
  available: boolean;
  createdAt: string;
}

export const GLOBAL_CONVERSION_RATE = 562.79; // FCFA / USD

const generateID = () => {
  return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function (c) {
    const r = (Math.random() * 16) | 0,
      v = c == "x" ? r : (r & 0x3) | 0x8;
    return v.toString(16);
  });
};

export const shopProductsRaw = [
  {
    id: generateID(),
    title: "ChatGPT Plus Personnel",
    description:
      "Abonnement ChatGPT Plus personnel avec accès à GPT-4, réponses plus rapides et accès prioritaire aux nouvelles fonctionnalités.",
    image: "/img/shop/chatgpt.png",
    price: 14.22,
    officialPrice: 20,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["ChatGPT", "GPT-4", "IA", "AI", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "ChatGPT Plus Partagé",
    description:
      "Abonnement ChatGPT Plus partagé entre plusieurs utilisateurs. Accès à GPT-4, réponses plus rapides. Solution économique pour petits groupes.",
    image: "/img/shop/chatgpt.png",
    price: 5.33,
    officialPrice: 20,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["ChatGPT", "GPT-4", "IA", "AI", "Partagé", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: true,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "ChatGPT Pro 5x",
    description:
      "Abonnement ChatGPT Pro avec 5x plus d'usage. Idéal pour équipes ou familles. Accès à GPT-4, réponses plus rapides et priorité.",
    image: "/img/shop/chatgpt.png",
    price: 62,
    officialPrice: 118,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["ChatGPT", "GPT-4", "IA", "AI", "Équipe", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "ChatGPT Pro 20x",
    description:
      "Abonnement ChatGPT Pro avec 20x plus d'usage. Solution entreprise pour grandes équipes. Accès GPT-4, priorité maximale.",
    image: "/img/shop/chatgpt.png",
    price: 100,
    officialPrice: 200,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["ChatGPT", "GPT-4", "IA", "AI", "Entreprise", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "ChatGPT Go",
    description:
      "Abonnement ChatGPT Go pour usage intensif. Accès étendu et prioritaire. Parfait pour professionnels.",
    image: "/img/shop/chatgpt.png",
    price: 4.44,
    officialPrice: 5,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["ChatGPT", "GPT-4", "IA", "AI", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "ChatGPT Go",
    description:
      "Abonnement ChatGPT Go pour usage intensif. Accès étendu et prioritaire. Parfait pour professionnels.",
    image: "/img/shop/chatgpt.png",
    price: 35,
    officialPrice: 5 * 12,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["ChatGPT", "GPT-4", "IA", "AI", "Dev", "Productivité"],
    duration: "1 an",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Claude Team (1.25x)",
    description:
      "Abonnement Claude Team avec 1.25x plus d'avantages que le Claude Pro standard. Idéal pour collaborer.",
    image: "/img/shop/claude.svg",
    price: 17.77,
    officialPrice: 20,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Claude", "Anthropic", "Team", "IA", "AI", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Claude Team Premium (6.25x)",
    description:
      "Abonnement Claude Team Premium avec 6.25x plus d'avantages que le Claude Pro standard. Puissance maximale pour équipes.",
    image: "/img/shop/claude.svg",
    price: 88.84,
    officialPrice: 100,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: [
      "Claude",
      "Anthropic",
      "Team",
      "Premium",
      "IA",
      "AI",
      "Dev",
      "Productivité",
    ],
    duration: "1 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Google AI Pro 5TB - 18 mois",
    description:
      "Abonnement Google AI Pro avec 5TB de stockage cloud pour 18 mois. Meilleur tarif long terme.",
    image: "/img/shop/googleone.jpg",
    price: (67.6 + 26) * 0.84,
    officialPrice: 19.99 * 18,
    currency: "F.CFA",
    primaryTag: "Cloud",
    tags: ["Google One", "Cloud", "Stockage", "IA", "AI", "Productivité"],
    duration: "18 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Google AI Pro",
    description:
      "Abonnement Google AI Pro + 5TB de stockage Google One avec accès à l'IA multimodale avancée. Capacités de reasoning et de génération supérieures.",
    image: "/img/shop/gemini.png",
    price: 26,
    officialPrice: 19.99 * 4,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Gemini", "Google", "IA", "AI", "Dev", "Cloud", "Productivité"],
    duration: "4 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Google AI Pro",
    description:
      "Abonnement Google AI Pro + 5TB de stockage Google One avec accès à l'IA multimodale avancée. Capacités de reasoning et de génération supérieures.",
    image: "/img/shop/gemini.png",
    price: 67.6,
    officialPrice: 19.99 * 12,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Gemini", "Google", "IA", "AI", "Dev", "Cloud", "Productivité"],
    duration: "1 an",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Veo 3 (45.000 Crédits)",
    description:
      "Abonnement Veo 3 Ultra avec 45,000 crédits IA pour génération vidéo. Créations de haute qualité, résolution 4K.",
    image: "/img/shop/veo.png",
    price: 60,
    officialPrice: 400,
    currency: "F.CFA",
    primaryTag: "Vidéo IA",
    tags: ["Veo", "IA", "AI", "Vidéo", "Génération", "Créativité"],
    duration: "1 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Google AI Ultra (25.000 Crédits)",
    description:
      "Abonnement Google AI Ultra + Antygravity et pour génération d'images et vidéos. Créations artistiques de haute qualité.",
    image: "/img/shop/googleantigravity.svg",
    price: 45,
    officialPrice: 249,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Antigravity", "IA", "AI", "Images", "Vidéos", "Dev", "Créativité"],
    duration: "1 an",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Super Grok",
    description:
      "Abonnement Super Grok de xAI avec IA avancée. Capacités de reasoning supérieures et multimodalité.",
    image: "/img/shop/grok.png",
    price: 21.32,
    officialPrice: 30,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Grok", "xAI", "IA", "AI", "Dev", "Productivité"],
    duration: "1 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Super Grok",
    description:
      "Abonnement Super Grok de xAI avec IA avancée. Capacités de reasoning supérieures et multimodalité.",
    image: "/img/shop/grok.png",
    price: 140,
    officialPrice: 30 * 8,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Grok", "xAI", "IA", "AI", "Dev", "Productivité"],
    duration: "8 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Super Grok Heavy",
    description:
      "Abonnement Super Grok de xAI avec IA avancée. Capacités de reasoning supérieures et multimodalité.",
    image: "/img/shop/grok.png",
    price: 170,
    officialPrice: 30 * 11,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Grok", "xAI", "IA", "AI", "Dev", "Productivité"],
    duration: "11 mois",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Perplexity Pro",
    description:
      "Abonnement Perplexity Pro pour la recherche IA avancée. Accès aux modèles GPT-4, Claude 3, et recherche en temps réel sur le web.",
    image: "/img/shop/perplexity.webp",
    price: 62.73,
    officialPrice: 17 * 12,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Perplexity", "IA", "AI", "Recherche", "Productivité", "Dev"],
    duration: "1 an",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Higgsfield Starter (200 Crédits)",
    description:
      "Abonnement Higgsfield Starter pour la génération vidéo IA sociale. Créez des vidéos virales en quelques secondes.",
    image: "/img/shop/higgsfield.png",
    price: 10.66,
    officialPrice: 15,
    currency: "F.CFA",
    primaryTag: "Vidéo IA",
    tags: ["Higgsfield", "Vidéo", "IA", "AI", "Génération", "Créativité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Kling Standard (1,100 Crédits)",
    description:
      "Abonnement Kling Standard avec 1,100 crédits IA pour génération vidéo. Créations vidéo de haute qualité.",
    image: "/img/shop/kling.png",
    price: 12.86,
    officialPrice: 20,
    currency: "F.CFA",
    primaryTag: "Vidéo IA",
    tags: ["Kling", "IA", "AI", "Vidéo", "Génération", "Créativité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Eleven Labs Creator",
    description:
      "Abonnement Eleven Labs Creator pour synthèse vocale IA. Voix ultra-réalistes, cloning vocal, multiple langues.",
    image: "/img/shop/elevenlabs.png",
    price: 15.99,
    officialPrice: 22,
    currency: "F.CFA",
    primaryTag: "Audio IA",
    tags: ["Eleven Labs", "IA", "AI", "Audio", "Voix", "Créativité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Figma Professional",
    description:
      "Abonnement Figma Professional pour le design collaboratif. Projets illimités, historique des versions, export haute résolution.",
    image: "/img/shop/figma.png",
    price: 96,
    officialPrice: 144,
    currency: "F.CFA",
    primaryTag: "Design",
    tags: ["Figma", "Design", "Collaboratif", "Créativité", "Dev"],
    duration: "1 an",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Notion Plus",
    description:
      "Abonnement Notion Plus pour la productivité. Uploads illimités, version history, collaboration avancée.",
    image: "/img/shop/notion.webp",
    price: 7.99,
    officialPrice: 10,
    currency: "F.CFA",
    primaryTag: "Productivité",
    tags: ["Notion", "Productivité", "Organisation", "Cloud"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Microsoft 365",
    description:
      "Abonnement Microsoft 365 pour la bureautique. Word, Excel, PowerPoint, Outlook, 1TB OneDrive.",
    image: "/img/shop/m365.png",
    price: 17.16,
    officialPrice: 59.94,
    currency: "F.CFA",
    primaryTag: "Bureautique",
    tags: ["Microsoft", "Office", "Cloud", "Bureautique", "Productivité"],
    duration: "6 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Cursor Pro 1 an",
    description:
      "Abonnement Cursor Pro annuel. L'IDE IA ultime pour coder plus vite. Accès illimité à Claude 3.5 Sonnet et GPT-4o.",
    image: "/img/shop/cursor.png",
    price: 62.58,
    officialPrice: 192,
    currency: "F.CFA",
    primaryTag: "Développement",
    tags: ["Cursor", "IDE", "IA", "AI", "Dev", "Développement", "Annuel"],
    duration: "1 an",
    isShared: false,
    featured: true,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Kiro Pro",
    description:
      "Abonnement Kiro Pro pour l'optimisation et le développement IA. Accès complet aux outils avancés.",
    image: "/img/shop/kiro.svg",
    price: 9,
    officialPrice: 20,
    currency: "F.CFA",
    primaryTag: "IA",
    tags: ["Kiro", "Dev", "IA", "AI"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "Suno Pro",
    description:
      "Abonnement Suno Pro pour la génération de musique IA. 2,500 crédits par mois, usage commercial inclus.",
    image: "/img/shop/suno.jpg",
    price: 8.88,
    officialPrice: 10,
    currency: "F.CFA",
    primaryTag: "Audio IA",
    tags: ["Suno", "Musique", "IA", "AI", "Créativité"],
    duration: "1 mois",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-15T00:00:00Z",
  },
  {
    id: generateID(),
    title: "CapCut Pro (650 Crédits AI + 1 TB Stockage)",
    description:
      "Abonnement CapCut Pro avec 650 Crédits AI + 1 TB Stockage pour l'édition vidéo. Effets premium, sans watermark, export HD illimité.",
    image: "/img/shop/capcut.jpg",
    price: 70,
    officialPrice: 89.99,
    currency: "F.CFA",
    primaryTag: "Vidéo",
    tags: ["CapCut", "Édition", "Mobile", "Vidéo", "Créativité", "IA", "AI"],
    duration: "1 an",
    isShared: false,
    featured: false,
    available: true,
    createdAt: "2026-05-08T00:00:00Z",
  },
];

export const shopProducts = shopProductsRaw.map((product) => ({
  ...product,
  price: Math.round(product.price * GLOBAL_CONVERSION_RATE),
  officialPrice: product.officialPrice
    ? Math.round(product.officialPrice * GLOBAL_CONVERSION_RATE)
    : undefined,
}));
