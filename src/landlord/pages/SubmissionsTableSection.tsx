...
export function SubmissionsTableSection() {
  const [page, setPage] = useState(1);
  const { data: tableData, isLoading, mutate } = useSWR(
    dataTableKey("submissions", page, pageSize),
    fetchLandlordTable,
  );
  ...
  return (
    <div className="grid gap-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h2 className="text-2xl font-semibold tracking-[-0.02em]">Submissions</h2>
          <p className="mt-1 text-sm text-black/45">Demandes entrantes</p>
        </div>
        <Button
          variant="outline"
          asIcon
