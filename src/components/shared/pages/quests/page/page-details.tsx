...
export function PageDetails({ descriptionTop, descriptionBottom, previewValueCard, useMarkdown }: PageDetailsProps) {
    ...
    <section className="relative w-full">
      <div className="relative w-full max-w-2xl mx-auto">
        <div className="flex flex-col w-full max-w-[90%] mx-auto items-start justify-center gap-4 relative">
          <h2 className="relative self-stretch h3 font-normal">{t("heading_presentation")}</h2>

          <div className="flex flex-col gap-8 md:gap-12 text-b-white-invert-thr font-normal text-base">
            {/* Top description */}
            {descriptionTop && <FormattedText useMarkdown={useMarkdown}>{descriptionTop}</FormattedText>}
            ...
            {/* Bottom description */}
            {descriptionBottom && <FormattedText useMarkdown={useMarkdown}>{descriptionBottom}</FormattedText>}
          </div>
        </div>
      </div>
    </section>
