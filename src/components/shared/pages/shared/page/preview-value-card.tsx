...
export function PreviewValueCard({ icon, badge, title, description, className }: CardInfoProps) {
  return (
    <Card
      className={cn(
    ...
    >
      <CardContent className="grid grid-cols-1 size-full p-4 gap-2">
        <PreviewValueCardDetails icon={icon} badge={badge} title={title} description={description} />
      </CardContent>
    </Card>
  )
}
