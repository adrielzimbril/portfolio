...
type PreviewContent = TextPreviewContent | ImagePreviewContent | VideoPreviewContent | CustomPreviewContent

/**
 * Function to render preview content
 */
export function HeaderPreviewCard({ content, type }: { content: PreviewContent; type?: ResourceType }) {
  switch (content.type) {
        ...
        <div className="relative size-full mx-auto transition-all duration-800 hover:scale-105">
          <div className="relative size-full pointer-events-none">
            <h3 className="h2 w-full relative mb-4">
              {content.emoji && (
                <>
                  <span className={cn("inline-block h2 px-2 py-5 rounded-full bg-sh-white")}>{content.emoji}</span>
                  <br />
                </>
              )}
              {content.title}
            </h3>
            {content.subtitle && <p className="relative h4 text-zinc-400">{content.subtitle}</p>}
          </div>
        </div>
      )
    ...
    case PreviewContentType.VIDEO:
      return (
        <div className="relative w-full h-full flex flex-col items-center justify-center">
          ...
          />
          {type && <PreviewIcon resourceType={type} />}
          {content.caption && <p className="text-zinc-400 text-center mt-4 text-lg">{content.caption}</p>}
        </div>
      )
...
export type { TextPreviewContent, ImagePreviewContent, VideoPreviewContent, CustomPreviewContent, PreviewContent }
