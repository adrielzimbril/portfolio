...
interface StatCardWithDecorationProps {
  label: string;
  value: number | string;
  suffix?: string;
...
export function StatCardWithDecoration({
  label,
  value,
  suffix,
  icon,
...
}: StatCardWithDecorationProps) {
  const { shouldReduceAnimations } = usePerformanceMode();
  const [isHovered, setIsHovered] = useState(false);
  const [displayValue, setDisplayValue] = useState(
    typeof value === "number" && shouldReduceAnimations ? value : 0,
  );

  useEffect(() => {
    if (typeof value !== "number" || shouldReduceAnimations) {
      return;
    }
