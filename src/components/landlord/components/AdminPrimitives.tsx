...
export function AdminCard({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <Card className={cn("rounded-xl border border-black/8 bg-white shadow-[0_1px_0_rgba(17,25,31,0.04)]", className)}>
      {children}
    </Card>
  )
}
...
export function EmptyState({
  ...
  return (
    <div className="flex min-h-64 flex-col items-center justify-center rounded-[20px] border border-dashed border-black/10 bg-black/2 p-8 text-center">
      ...
      <p className="mt-2 max-w-sm text-sm leading-6 text-black/50">{description}</p>
    </div>
  )
}
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent size="md" variant="modern">
        ...
        <DialogSeparator />
        <DialogFooter>
          <Button type="button" variant="outline" asPointer onClick={() => onOpenChange(false)} disabled={loading}>
            {tShared("cancel")}
          </Button>
  ...
  return (
    <AdminCard className="p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className={cn("flex size-11 items-center justify-center rounded-2xl", toneClass[tone])}>
          <Icon size={19} />
        </div>
      </div>
    </AdminCard>
...
export function TablePager({
  ...
  return (
    <div className="flex items-center justify-between gap-3 border-t border-black/8 px-5 py-4 text-sm">
      <Badge variant="white">{tShared("page", { page, total: totalPages })}</Badge>
      <div className="flex items-center gap-2">
        <Button variant="outline" size="xs" asPointer disabled={page <= 1} onClick={() => onPageChange(page - 1)}>
          {tShared("previous")}
        </Button>
        <Button
          variant="outline"
