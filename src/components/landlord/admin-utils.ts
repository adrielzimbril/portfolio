...
export const registrationsKey = (selectedQuest: string, page: number, pageSize = 10) => {
...
export const submissionsKey = (selectedQuest: string, page: number, pageSize = 10) => {
  const params = new URLSearchParams()
  if (selectedQuest !== "all") params.set("slug", selectedQuest)
  params.set("page", String(page))
  params.set("pageSize", String(pageSize))
...
export const participantsKey = (selectedQuest: string, page: number, pageSize = 10, type?: string) => {
...
export async function fetchLandlordTable(url: string): Promise<LandlordTableResponse> {
...
export async function fetchParticipants(url: string): Promise<LandlordTableResponse> {
  return fetchLandlordTable(url)
}

export async function fetchMessages(url: string): Promise<LandlordTableResponse> {
  return fetchLandlordTable(url)
}

export async function deleteParticipant(type: "register" | "submission", id: string) {
  const baseUrl = type === "submission" ? landlordApiRoutes.quests.submissions : landlordApiRoutes.quests.registrations
...
export async function updateParticipant(type: "register" | "submission", id: string, data: any) {
  const baseUrl = type === "submission" ? landlordApiRoutes.quests.submissions : landlordApiRoutes.quests.registrations
  const url = `${baseUrl}/${id}`
  const response = await fetch(url, {
    method: "PATCH",
