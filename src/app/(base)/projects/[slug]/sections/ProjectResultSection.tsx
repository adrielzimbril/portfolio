    ...
    <>
      ...
      >
        {results.map((result, index) => (
          <Card
            key={index}
            className=" size-full max-w-[90%] md:max-w-full bg-b-base squircle-4xl/60 md:squircle-4xl/60  border-0 overflow-hidden"
          >
            <CardContent className="grid grid-cols-1 size-full p-4 gap-2">
              <div
                className={cn(
                  "flex relative flex-col gap-6 md:gap-4 min-h-60 items-start justify-between p-6 md:p-8 squircle-3xl/100 bg-sh-white overflow-hidden",
                )}
              >
