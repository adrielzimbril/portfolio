  ...
  return (
    <div className="flex flex-col gap-8 items-start justify-start relative">
      <>
        ...
        <h2 className="relative">Shiroshop</h2>
        ...
        <div className="flex items-center gap-4">
          <Link href={routes.projects.link} likeButton whileTap size="lg">
            Voir le projet 🦄
          </Link>
          <Link
            href={routes.contact.link}
...
export function HeaderSection() {
  ...
  return (
    <>
      ...
      <SectionBase
        isCallToAction
        className="flex-wrap col-span-2 md:[&>*]:flex-1"
      >
        <EmojiPlaceholder src="/image-1001.png" isMobileHidden />
        <ContentSection isPage />
      </SectionBase>
      <section className="h-[796px] relative w-full bg-sgreys-00">
              ...
              <div className="flex flex-wrap items-start gap-[6px] px-2 py-1 relative w-full bg-white rounded-lg overflow-hidden shadow-sm">
                {tags.map((tag, index) => (
                  <Badge
                    key={index}
                    variant="secondary"
                    className={`${tag.bgColor} rounded-[10px] px-3 py-1.5 backdrop-blur-[50px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(50px)_brightness(100%)] border-0 cursor-pointer transition-all duration-200 transform hover:scale-105 hover:shadow-md active:scale-95`}
