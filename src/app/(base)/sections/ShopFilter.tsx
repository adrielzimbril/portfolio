...
export function ShopFilter({ onFilteredProductsChange }: ShopFilterProps) {
  ...
  const categories = useMemo(
    () =>
      Array.from(new Set(shopProducts.map((p) => p.primaryTag))).sort(),
    [],
  );

  // Types remain static
  const types = useMemo(() => ["Personnel", "Partagé"], []);

  // Availability options
  const availabilities = useMemo(() => ["Disponible", "Indisponible"], []);
  ...
  // Pre-calculate normalized products for ultra-fast filtering
  const normalizedProducts = useMemo(() => {
    return shopProducts.map((p) => ({
      ...p,
      _normalizedTitle: normalize(p.title),
      _normalizedDescription: normalize(p.description),
      _normalizedPrimaryTag: normalize(p.primaryTag),
      _normalizedTags: p.tags.map((t) => normalize(t)),
    }));
  }, []);
  ...
  const getMatchesSearch = (
    ...
    const targetFields = [
      product._normalizedTitle,
      product._normalizedDescription,
      product._normalizedPrimaryTag,
      ...product._normalizedTags,
    ];

    // 1. Direct substring match (Strongest)
    if (targetFields.some((field) => field.includes(normalizedQuery)))
      return true;

    // 2. Word-by-word match (Look-alike)
    if (
      searchTerms.length > 0 &&
      searchTerms.some((term) => targetFields.some((field) => field.includes(term)))
    )
      return true;
    ...
    if (normalizedQuery.length > 2) {
      ...
      const queryInCategories = categories.some((cat) =>
        normalize(cat).includes(normalizedQuery),
      );
      if (
        queryInCategories &&
        product._normalizedPrimaryTag.includes(normalizedQuery)
      )
        return true;
    }
  ...
  // Calculate counts using memoization
  ...
  const performFiltering = useCallback(
    (
      products: typeof normalizedProducts,
      query: string,
      cat: string[],
      type: string | null,
      avail: string | null,
      ...
      return products
        .map((product) => {
          const matchesCategory = cat.length === 0 || cat.includes(product.primaryTag);
          const matchesType =
            !type ||
          ...
          const normalizedQuery = normalize(query);
          const isExactTitleMatch = query
            ? product._normalizedTitle.includes(normalizedQuery)
            : true;
          ...
          return {
            ...product,
            _isVisible:
              matchesCategory && matchesType && matchesAvailability && searchMatch,
  ...
  useEffect(() => {
    onFilteredProductsChangeRef.current(filteredProducts as any);
  }, [filteredProducts]);
