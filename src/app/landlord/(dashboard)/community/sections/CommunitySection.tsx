...
import { fetchMessages, formatDate, formatTime, messagesKey, normalizeMessage } from "@/components/landlord/admin-utils"
...
export function CommunitySection() {
  ...
  const { data: tableData, isLoading: loading } = useSWR(messagesKey(page, pageSize), fetchMessages)
                              ...
                              <div className="grid max-w-xl gap-1">
                                {entries.slice(0, 2).map(([locale, value]) => (
                                  <p key={locale} className="truncate text-black/65">
                                    <span className="mr-2 text-xs font-semibold uppercase text-black/35">{locale}</span>
                                    {value}
                                  </p>
                                ))}
                              </div>
