...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const adminEmails = ConfigValue.NEXT_PRIVATE_ADMIN_EMAILS?.split(",").map((e) => e.trim()) || []
    ...
    if (creator_avatar_url !== undefined) updateData.creator_avatar_url = creator_avatar_url

    logger.info("Update data:", updateData)

    const { data, error } = await client.from("community_wall").update(updateData).eq("id", id).select()
