...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch hub product links" }, { status: 500 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ error: "Failed to update hub product link" }, { status: 500 })
  }
}
