...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (scope === "summary") {
      const [
        { count: newsletterCount, error: e1 },
        cCourse,
        cEbook,
        cVideo,
        totalReq,
      ...
      if (e1 || cCourse.error || cEbook.error || cVideo.error || totalReq.error)
        throw (
          e1 || cCourse.error || cEbook.error || cVideo.error || totalReq.error
        );
    ...
    return json(
      { error: (e as Error)?.message || "Impossible to retrieve statistics" },
      500,
    );
  }
}
