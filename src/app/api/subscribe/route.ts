...
export async function POST(req: NextRequest) {
  ...
  const listIds = [generalId, productId].filter((n): n is number => !!n && !Number.isNaN(n))
  ...
  try {
    ...
    console.warn('Brevo add contact error:', e?.message || e)
  }

  return new Response(JSON.stringify({ success: true, alreadyExists }), { status: 200, headers: { 'Content-Type': 'application/json' } })
}
