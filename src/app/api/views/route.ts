...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ ...(wantResponse ? { count: result.total_views } : {}) }, { status: 200 })
  } catch (error: unknown) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { type = "page", slug = null, details = null, wantResponse = true } = body

    // Use the PostgreSQL RPC function to handle everything atomically
    const { data: analyticsData, error: rpcError } = await supabase.rpc("increment_page_analytics", {
      p_type: type,
      p_slug: slug,
      p_user_ip: ip,
    ...
    logger.error("Error incrementing analytics:", (error as Error)?.message || error)
    return NextResponse.json({ error: (error as Error)?.message || "UNKNOWN_ERROR" }, { status: 500 })
  }
}
