...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const { data: existingUser } = await db.from("users").select("*").eq("email", email).limit(1)
    ...
    const { error: registrationError } = await db.from("challenge_registrations").upsert(
      {
        challenge_slug: slug,
        user_id: userId,
        name,
    ...
    const { error: newsletterError } = await db.from("newsletter_subscribers").upsert(
      {
        user_id: userId,
    ...
    const listIds = [globalListId, registerListId].filter((id): id is number => Boolean(id) && !Number.isNaN(id))
