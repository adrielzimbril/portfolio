...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const { error: submissionError } = await db.from("challenge_submissions").upsert(
      {
        challenge_slug: slug,
        user_id: userId,
        name,
    ...
    const listIds = [globalListId, submissionsListId].filter((id): id is number => Boolean(id) && !Number.isNaN(id))
