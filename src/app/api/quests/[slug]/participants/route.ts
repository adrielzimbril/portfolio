...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const [registrationsRes, submissionsRes] = await Promise.all([
      db.from("challenge_registrations").select("email").eq("challenge_slug", slug),
    ...
    const normalized = (email: string | null | undefined) => (email ?? "").trim().toLowerCase()

    const registrationEmails = new Set<string>(
      (registrationsRes.data ?? []).map((row: { email?: string | null }) => normalized(row.email)).filter(Boolean),
    )

    const submissionEmails = new Set<string>(
      (submissionsRes.data ?? []).map((row: { email?: string | null }) => normalized(row.email)).filter(Boolean),
    )

    const uniqueParticipants = new Set<string>([...registrationEmails, ...submissionEmails])
