...
export function useNewsletterSubscribersCount() {
  const { data, error, isLoading, mutate } = useSWR(`${apiRoutes.stats.subscribers.link}?scope=newsletter`, fetchJSON, {
...
export function useProductTypeSubscribersCount(type: ResourceTypeKey) {
  const { data, error, isLoading, mutate } = useSWR(
    type ? `${apiRoutes.stats.subscribers.link}?scope=productType&type=${encodeURIComponent(type)}` : null, // null = skip fetch if no type
    fetchJSON,
  )
...
export function useProductSlugRequestsCount(slug: string) {
  const { data, error, isLoading, mutate } = useSWR(
    slug ? `${apiRoutes.stats.subscribers.link}?scope=productUrl&url=${encodeURIComponent(slug)}` : null, // null = skip fetch if no title
    fetchJSON,
  )
...
export function useQuestParticipantsStats(slug: string) {
  const { data, error, isLoading, mutate } = useSWR(slug ? apiRoutes.quests.participants(slug).link : null, fetchJSON)
