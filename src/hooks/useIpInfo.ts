...
const CountrySchema = z.object({
  ...
  phone_extra: z
    .object({
      ...
      national_destination_code_lengths: z.array(z.number()).nullable().optional(),
    })
    .optional(),
  flag: z.string(),
  flag_emoji: z.string(),
...
export async function getIpInfo<T = IpInfoResponse>(
  ...
  const { timeout = 10000, retryCount = 2, retryDelay = 1000, strictValidation = true, simplified = false } = options
    ...
    for (let attempt = 0; attempt <= retryCount; attempt++) {
      try {
        ...
        try {
          ...
          const finalData = simplified ? (transformToSummary(validatedData) as T) : (validatedData as T)
          ...
          if (validationError instanceof z.ZodError) {
            if (strictValidation) {
              return {
                data: null,
                error: `Données invalides: ${JSON.stringify(validationError.issues)}`,
                isValid: false,
              ...
              const finalData = simplified && rawResult ? (transformToSummary(rawResult) as T) : (rawResult as T)
...
export async function useGetIpInfo(ip?: string, simplified: boolean = false) {
  return simplified ? await getIpInfo<IpInfoSummary>(ip, { simplified: true }) : await getIpInfo(ip)
}
