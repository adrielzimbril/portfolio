...
import { defaultErrorMap, ZodErrorMap, ZodIssueCode, ZodParsedType } from "zod/v3"
    ...
    switch (issue.code) {
      ...
      case ZodIssueCode.invalid_string:
        if (typeof issue.validation === "object") {
          ...
          message = t(`zod.errors.invalid_string.${issue.validation}` as TranslationKey, {
            validation: t(`zod.validations.${issue.validation}` as TranslationKey),
          })
        }
        break
      ...
      case ZodIssueCode.custom: {
        ...
        message = t(`zod.errors.custom.${key}` as Parameters<typeof t>[0], (values as TranslationValues) ?? {})
        break
      }
